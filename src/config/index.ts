...
const config = {
  publicServer: {
    port: parseInt(process.env.PUBLIC_SERVER_PORT as string, 10) || 9001,
  },
  privateServer: {
  ...
  ai: {
    ...
    ai_chat_websocket:
      process.env.AI_CHAT_WEBSOCKET || 'http://localhost:7101',
  },
  ips: {
  ...
  fileService: {
    host: process.env.FILE_SERVICE_HOST || 'http://localhost:8866',
  },
  dataBoard: {
