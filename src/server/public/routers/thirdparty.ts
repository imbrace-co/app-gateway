    ...
    req.on('end', () => {
      ...
      if (rawBody) {
        try {
          ...
          logger.info('========= CAPTURED RAW BODY =========');
          logger.info('   Raw:', rawBody);
          logger.info('   Parsed:', (req as any).parsedBody);
          
          // Re-inject body stream cho proxy middleware
          req.body = (req as any).parsedBody;
        } catch (e) {
          logger.info('   Failed to parse body:', e);
          (req as any).rawBody = rawBody;
        }
      }
      next();
  ...
  createProxyMiddleware({
    ...
    on: {
      proxyReq: (proxyReq: ClientRequest, req: Request) => {
        logger.info(`========= proxy board_search:`);
...
router.use(
  ...
  createProxyMiddleware({
    ...
    on: {
      proxyReq: (proxyReq: ClientRequest, req: Request) => {
        logger.info(`========= proxy board - upload:`);
        logger.info(`   Original URL: ${req.originalUrl}`);
        logger.info(`   Request Path: ${req.path}`);
        logger.info(`   Target: ${config.backend.private_host}`);
        logger.info(
...
router.use(
  ...
  createProxyMiddleware({
    ...
    pathRewrite: (path, req) => {
      ...
      logger.info(`   [pathRewrite] ${path} -> ${newPath}`);
      return newPath;
    },
    followRedirects: false,
    ws: false,
    ...
    on: {
      proxyReq: (proxyReq: ClientRequest, req: Request) => {
        logger.info(`========= proxy /organization/:org_id/marketplaces:`);
        logger.info(`   Original URL: ${req.originalUrl}`);
        logger.info(`   Request Path: ${req.path}`);
        logger.info(`   Org ID Param: ${req.params.org_id}`);
        logger.info(`   Target: ${config.backend.private_host}`);
        logger.info(`   Method: ${req.method}`);
        logger.info(
          `   Content-Type: ${req.headers['content-type'] || 'NONE'}`
...
router.use(
  ...
  createProxyMiddleware({
    ...
    on: {
      proxyReq: (proxyReq: ClientRequest, req: Request) => {
        logger.info(`========= proxy /organization/:org_id/users/:user_id/schedulers:`);
        logger.info(`   Original URL: ${req.originalUrl}`);
        logger.info(`   Request Path: ${req.path}`);
        logger.info(`   Org ID Param: ${req.params.org_id}`);
        logger.info(`   User ID Param: ${req.params.user_id}`);
        logger.info(`   Target: ${config.backend.private_host}`);
        logger.info(`   Method: ${req.method}`);
        logger.info(
          `   Content-Type: ${req.headers['content-type'] || 'NONE'}`
...
router.use(
  ...
  createProxyMiddleware({
    ...
    on: {
      proxyReq: (proxyReq: ClientRequest, req: Request) => {
        logger.info(`========= proxy /organization/:org_id/channels:`);
        logger.info(`   Original URL: ${req.originalUrl}`);
        logger.info(`   Request Path: ${req.path}`);
        logger.info(`   Org ID Param: ${req.params.org_id}`);
        logger.info(`   Target: ${config.backend.private_host}`);
...
router.use(
  ...
  createProxyMiddleware({
    ...
    on: {
      proxyReq: (proxyReq: ClientRequest, req: Request) => {
        logger.info(`========= proxy conversations:`);
        logger.info(`   Original URL: ${req.originalUrl}`);
        logger.info(`   Request Path: ${req.path}`);
        logger.info(`   Target: ${config.backend.private_host}`);
        logger.info(`   Method: ${req.method}`);
...
router.use(
  ...
  createProxyMiddleware({
    ...
    on: {
      proxyReq: (proxyReq: ClientRequest, req: Request) => {
        logger.info(`========= board - upload:`);
        logger.info(`   Original URL: ${req.originalUrl}`);
        logger.info(`   Request Path: ${req.path}`);
        logger.info(`   Target: ${config.backend.private_host}`);
        logger.info(`   Forwarded URL: ${config.backend.private_host}${req.path}`);
        logger.info(`   Method: ${req.method}`);
