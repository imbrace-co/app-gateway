...
// Create a proxy for the Workflow backend API
const activePiecesBackendProxy = createProxyMiddleware({
    ...
    on: {
        proxyReq: (proxyReq: ClientRequest, req: Request) => {
            ...
            logger.info(`🔄 AP-BACKEND PROXY:`);
            logger.info(`   Original URL: ${req.originalUrl}`);
            logger.info(`   Forwarded URL: ${config.activepieces.backend_host}${req.path}`);
        ...
        proxyRes: (proxyRes, req: Request) => {
            logger.debug(`Workflow Backend response: ${req.method} ${req.path} -> ${proxyRes.statusCode}`);
        },

        error: (err, req: Request) => {
            logger.error(`Workflow Backend proxy error for ${req.method} ${req.path}:`, err.message);
        }
    },
});

// Export the Workflow backend service with standard authentication middleware
