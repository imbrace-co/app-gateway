...
const validateApiToken = async (apiKey: string): Promise<{
  ...
  // that communicates with your backend service
  const url = `${config.backend.private_host}/v1/api_key_token/${apiKey}`;
  try {
    ...
    if (!data.permissions || typeof data.permissions !== 'object') {
      console.warn('No permissions found in token response');
      data.permissions = {};
    }
    ...
    console.error('API Token Validation Error:', axiosError);
...
const getApiPath = (path: string): string => {
  ...
  console.debug(`No specific API path match for: ${normalizedPath}`);
  return ''
};
...
export const apiAuthenticate = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    console.debug(`API access: ${req.method} ${apiPath} by user ${user_id} from org ${org_id}`);
